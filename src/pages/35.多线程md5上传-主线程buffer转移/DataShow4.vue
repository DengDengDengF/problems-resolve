<template>
  ...
  <el-scrollbar :height="500" always style="padding-right:20px">
    <div v-for="item in  fileList" class="lists">
      <div class="lists-item">
        ...
        <div class="right">
          <span :style="{color:statusHash[item.status].color}">{{
              statusHash[item.status].value
            }}-{{ item.md5 }}</span>
  ...
  <span style="color: red;cursor:pointer" v-if="errorList.length" @click="retry">重试</span>
...
const statusHash:Record<string, any> = {
...
/**初始化文件
 * status:0未计算 1计算中 2计算完成 3计算失败*/
const initFile = (file: File) => ({file, md5: '', errorMsg: '', uid: crypto.randomUUID(), status: 0})
//全部选中
